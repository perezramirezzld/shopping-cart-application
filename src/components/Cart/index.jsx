...
const Cart = () => {
  ...
  // Función para calcular la cantidad total de galletas de todos los productos en el carrito
  const getTotalCookies = () => {
    let totalCookies = 0;
    for (const item of cartItems) {
      if (item.name === "coffe small") {
        totalCookies += item.amount * 0;
      } else if (item.name === "coffe medium") {
        totalCookies += item.amount * 3;
      } else if (item.name === "coffe large") {
        totalCookies += item.amount * 6;
      }else if (item.name === "coffe jumbo") {
        totalCookies += item.amount * 1;
      }
    }
    return totalCookies;
  };
    ...
    <div className={styles.cartContainer}>
      ...
      {cartItems && cartOpen && (
        <div className={styles.cart}>
          ...
          <h3 className={styles.cookies}>Galletas: {totalCookies}</h3>
